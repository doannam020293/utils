...
def train(all_train_data, ft_names=None, skip_col=None, label_col="label", cross_val=True, print_corr=True):
    ...
    if skip_col == None:
        skip_col = ["phone_number", "local_id", "disburse_date", "date", "sex", "replied", "label_col", label_col,
                    "label_value", "month",
    ...
    print("train_test_split size `", train_data.shape, test_data.shape)
...
def train_catboost(
        all_train_data,
        ft_names=None,
        skip_col=["phone_number", "disburse_date", "label", "label_value"],
        object_col=None,
        label_col="label",
        cross_val=True):
    if ft_names == None:
    ...
    if object_col is None:
        object_col = all_train_data.select_dtypes(include=["object", ]).columns.tolist()
    object_col_indices = np.where(all_train_data[ft_names].columns.isin(object_col))[0]
    print("object_col", object_col)
    print("object_col_indices", object_col_indices)
    ...
    model = CatBoostClassifier(thread_count=32, cat_features=object_col_indices, verbose=False)
    ...
    print(df_cor.head())
    if cross_val:
        model = CatBoostClassifier(thread_count=32, cat_features=object_col_indices,
                                   verbose=False, iterations=1000)
        scores = cross_val_score(model, X, Y, cv=5, scoring="roc_auc")
        scores = [np.round(a * 100, 2) for a in scores]
        print("auc cross validate", scores)
        print("mean auc", np.mean(scores), "max - min", np.max(scores) - np.min(scores), "std", np.std(scores))
